...
'''
def tmp2tiff(input_file, output_file, color='rgb'):
    ...
    if color == 'rgb':	
        tifffile.imwrite(output_file, im, dtype=np.float32, photometric=color) # FIXME: NOT WORKING W/ PIL (planarconfig = 'contig' ?)	
        #imageio.imwrite(output_file, im, format='tif') - NOT WORKING EITHER	
    else:	
        tifffile.imwrite(output_file, im, photometric=color)